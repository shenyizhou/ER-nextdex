...
type Provider = "glossary" | "google" | "openai"
type Domain =
    | "ability_name"
    | "ability_desc"
    | "move_name"
...
) => Promise<GoogleTranslateResult | GoogleTranslateResult[] | Array<GoogleTranslateResult | null>>

interface ExternalGlossary {
...
interface TranslationStats {
    ...
    changedByGlossary: number
    changedByOpenAI: number
    unchanged: number
    pending: number
}
...
interface TranslationState {
    options: CliOptions
    glossary: ExternalGlossary
    cache: Record<string, string>
    pendingByKey: Map<string, PendingEntry[]>
    unresolvedNames: Record<string, Set<string>>
...
const evoKindMap: Record<string, string> = {
    EVO_LEVEL: "等级进化",
...
const scriptedEncounterMap: Record<string, string> = {
...
type RegexReplacement = string | ((substring: string, ...groups: string[]) => string)

const regexReplacements: Array<[RegExp, RegexReplacement]> = [
    [/\b([A-Z][a-z]+)-type\b/g, (_substring: string, match: string) => `${typeMap[match] || match}属性`],
    [/\b([A-Z][a-z]+) types\b/g, (_substring: string, match: string) => `${typeMap[match] || match}属性`],
    [/\b([A-Z][a-z]+) type\b/g, (_substring: string, match: string) => `${typeMap[match] || match}属性`],
...
function parseArgs(argv: string[]): CliOptions {
    ...
    for (let index = 2; index < argv.length; index++) {
        ...
        if (!next) throw new Error(`${arg} 缺少参数值`)
        if (arg === "--input") baseOptions.input = path.resolve(next)
        else if (arg === "--output") baseOptions.output = path.resolve(next)
        else if (arg === "--cache") baseOptions.cache = path.resolve(next)
        else if (arg === "--glossary") baseOptions.glossary = path.resolve(next)
        else if (arg === "--template") baseOptions.template = path.resolve(next)
        else if (arg === "--report") baseOptions.report = path.resolve(next)
        else if (arg === "--provider") baseOptions.provider = next as Provider
        else if (arg === "--limit") baseOptions.limit = Number(next)
...
function createEmptyGlossary(): ExternalGlossary {
    return {
        exact: {},
        species: {},
        abilities: {},
...
async function readJsonIfExists<T>(filePath: string | undefined, fallback: T): Promise<T> {
...
function replaceWholeToken(text: string, search: string, replacement: string): string {
    let currentIndex = 0
    let output = ""
...
function getDomain(pathParts: string[]): Domain {
    const root = pathParts[0]
    const tail = pathParts[pathParts.length - 1]
    if (root === "abilities" && tail === "name") return "ability_name"
    if (root === "abilities" && tail === "desc") return "ability_desc"
    if (root === "moves" && tail === "name") return "move_name"
    if (root === "moves" && tail === "sName") return "move_short_name"
    if (root === "moves" && tail === "desc") return "move_desc"
    if (root === "moves" && tail === "lDesc") return "move_long_desc"
    if (root === "species" && tail === "name") return "species_name"
    if (root === "species" && tail === "desc") return "species_desc"
    if (root === "items" && tail === "name") return "item_name"
    if (root === "mapsT") return "map_name"
    if (root === "trainers" && tail === "name") return "trainer_name"
    if (root === "tclassT") return "trainer_class"
    if (root === "typeT") return "type_name"
    if (root === "targetT") return "target_name"
    if (root === "flagsT") return "flag_name"
    if (root === "effT") return "effect_name"
    if (root === "splitT") return "split_name"
    if (root === "eggT") return "egg_group"
    if (root === "growT") return "growth_name"
    if (root === "colT") return "color_name"
    if (root === "evoKindT") return "evo_kind"
    if (root === "natureT") return "nature_name"
    if (root === "scriptedEncoutersHowT") return "scripted_encounter"
    return "generic"
}

function getDomainGlossary(domain: Domain, glossary: ExternalGlossary): Record<string, string> {
    if (domain === "species_name") return glossary.species || {}
    if (domain === "ability_name") return glossary.abilities || {}
    if (domain === "move_name" || domain === "move_short_name") return glossary.moves || {}
    if (domain === "item_name") return glossary.items || {}
    if (domain === "map_name") return glossary.maps || {}
    if (domain === "trainer_class") return glossary.trainerClasses || {}
    if (domain === "trainer_name") return glossary.trainers || {}
    return glossary.exact || {}
}

function getBuiltinExactForDomain(domain: Domain): Record<string, string> {
    if (domain === "type_name") return typeMap
    if (domain === "target_name") return targetMap
    if (domain === "split_name") return splitMap
    if (domain === "egg_group") return eggGroupMap
    if (domain === "color_name") return colorMap
    if (domain === "evo_kind") return evoKindMap
    if (domain === "nature_name") return natureMap
    if (domain === "scripted_encounter") return scriptedEncounterMap
    return {}
}

function applyDeterministicTranslation(text: string, domain: Domain, glossary: ExternalGlossary): { text: string, changed: boolean, needsRemote: boolean } {
    let translated = text
    ...
    translated = translated.replace(/\b([A-Z][a-z]+)-type\b/g, (_substring, match: string) => `${typeMap[match] || match}属性`)
    translated = translated.replace(/\b([A-Z][a-z]+) types\b/g, (_substring, match: string) => `${typeMap[match] || match}属性`)
    translated = translated.replace(/\b([A-Z][a-z]+) type\b/g, (_substring, match: string) => `${typeMap[match] || match}属性`)
...
function buildCacheKey(domain: Domain, text: string): string {
    return `${domain}\n${text}`
}

function ensureSet(record: Record<string, Set<string>>, key: string): Set<string> {
    if (!record[key]) record[key] = new Set<string>()
    return record[key]
}

function markUnresolved(domain: Domain, text: string, state: TranslationState) {
    if (!text.trim()) return
    if (domain === "species_name") ensureSet(state.unresolvedNames, "species").add(text)
    else if (domain === "ability_name") ensureSet(state.unresolvedNames, "abilities").add(text)
    else if (domain === "move_name" || domain === "move_short_name") ensureSet(state.unresolvedNames, "moves").add(text)
    else if (domain === "item_name") ensureSet(state.unresolvedNames, "items").add(text)
    else if (domain === "map_name") ensureSet(state.unresolvedNames, "maps").add(text)
    else if (domain === "trainer_class") ensureSet(state.unresolvedNames, "trainerClasses").add(text)
    else if (domain === "trainer_name") ensureSet(state.unresolvedNames, "trainers").add(text)
}

function addPending(entry: PendingEntry, state: TranslationState) {
...
function translateNode(node: JsonValue, pathParts: string[], parent: JsonObject | JsonValue[] | null, key: string | number | null, state: TranslationState): JsonValue {
    if (typeof node === "string") {
        ...
        if (!translated.needsRemote || state.options.provider === "glossary") {
            if (translated.changed) state.stats.changedByGlossary += 1
            else state.stats.unchanged += 1
            if (!translated.changed && /_/.test(node) === false) {
                markUnresolved(domain, node, state)
            }
            return translated.text
        }
        if (parent === null || key === null) return translated.text
        ...
        if (translated.changed) state.stats.changedByGlossary += 1
        else markUnresolved(domain, node, state)
        return translated.text
    }
...
function buildGlossaryPrompt(glossary: ExternalGlossary): string {
...
async function translateBatchWithOpenAI(batch: Array<{ key: string, domain: Domain, text: string }>, options: CliOptions, glossary: ExternalGlossary): Promise<Record<string, string>> {
    if (!options.apiKey) throw new Error("使用 openai provider 时必须提供 OPENAI_API_KEY")
    ...
    const translatedEntries = JSON.parse(normalized) as Array<{ key: string, translation: string }>
...
function loadGoogleTranslate(): GoogleTranslateFn {
    const googleTranslateModule = require("google-translate-api-x") as {
        default?: GoogleTranslateFn
        translate?: GoogleTranslateFn
    } | GoogleTranslateFn
    if (typeof googleTranslateModule === "function") return googleTranslateModule
    if (googleTranslateModule.default) return googleTranslateModule.default
    if (googleTranslateModule.translate) return googleTranslateModule.translate
    throw new Error("无法加载 google-translate-api-x")
}

async function translateBatchWithGoogle(batch: Array<{ key: string, domain: Domain, text: string }>): Promise<Record<string, string>> {
    const translate = loadGoogleTranslate()
    const mapped: Record<string, string> = {}
    const safeTranslateSingle = async (text: string): Promise<string> => {
        try {
    ...
    let results: Array<GoogleTranslateResult | null> = []
    try {
...
function chunkPendingEntries(entries: Array<{ key: string, domain: Domain, text: string }>): Array<Array<{ key: string, domain: Domain, text: string }>> {
    const chunks: Array<Array<{ key: string, domain: Domain, text: string }>> = []
    let currentChunk: Array<{ key: string, domain: Domain, text: string }> = []
    let currentLength = 0
...
async function flushPending(state: TranslationState): Promise<void> {
    ...
    for (const [cacheKey, entries] of state.pendingByKey.entries()) {
        ...
        for (const entry of entries) {
            ;(entry.parent as any)[entry.key] = translated
        }
        if (translated !== entries[0].text) state.stats.changedByOpenAI += entries.length
        else state.stats.unchanged += entries.length
    }
}

function buildTemplate(state: TranslationState): Record<string, Record<string, string>> {
...
function applyStructuredGlossaryOverrides(sourceData: JsonObject, translatedData: JsonObject, glossary: ExternalGlossary) {
    ...
    for (let index = 0; index < Math.min(sourceAbilities.length, translatedAbilities.length); index++) {
        ...
        if (!sourceAbility || typeof sourceAbility !== "object" || Array.isArray(sourceAbility)) continue
        if (!translatedAbility || typeof translatedAbility !== "object" || Array.isArray(translatedAbility)) continue
        ...
        if (glossaryName) translatedAbility.name = glossaryName
        if (glossaryDesc) translatedAbility.desc = glossaryDesc
    }
    ...
    for (let index = 0; index < Math.min(sourceMoves.length, translatedMoves.length); index++) {
        ...
        if (!sourceMove || typeof sourceMove !== "object" || Array.isArray(sourceMove)) continue
        if (!translatedMove || typeof translatedMove !== "object" || Array.isArray(translatedMove)) continue
        ...
        if (glossaryDesc && typeof translatedMove.desc === "string") translatedMove.desc = glossaryDesc
    }
    ...
    for (let index = 0; index < Math.min(sourceSpecies.length, translatedSpecies.length); index++) {
        ...
        if (!sourceSpecie || typeof sourceSpecie !== "object" || Array.isArray(sourceSpecie)) continue
        if (!translatedSpecie || typeof translatedSpecie !== "object" || Array.isArray(translatedSpecie)) continue
        const sourceName = typeof sourceSpecie.name === "string" ? sourceSpecie.name : ""
        const glossaryName = glossary.species?.[sourceName]
        if (glossaryName) translatedSpecie.name = glossaryName
    }
}

function buildReport(state: TranslationState, outputPath: string): JsonObject {
    return {
        ...
        unresolved: Object.fromEntries(
            Object.entries(state.unresolvedNames).map(([key, values]) => [key, [...values].sort((left, right) => left.localeCompare(right))])
        )
    }
}
...
async function main() {
    ...
    const glossary = await readJsonIfExists<ExternalGlossary>(options.glossary, createEmptyGlossary())
    const cache = await readJsonIfExists<Record<string, string>>(options.cache, {})
    const rawInput = await fs.promises.readFile(options.input, "utf8")
    const sourceData = JSON.parse(rawInput) as JsonObject
    const state: TranslationState = {
        options,
        glossary,
        cache,
        pendingByKey: new Map<string, PendingEntry[]>(),
        unresolvedNames: {},
        stats: {
            scannedStrings: 0,
            changedByGlossary: 0,
    ...
    const translatedData = translateNode(sourceData, [], null, null, state) as JsonObject
